...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if db.is_user_approved(user_id):
        await update.message.reply_text(
            f"Привет, {user.first_name}! Ты уже одобрен и можешь пользоваться ботом.\n\n"
            "После каждого молодежного тебе придет опрос для оценки встречи."
        )
    elif db.is_user_pending(user_id):
        await update.message.reply_text(
            "Твой запрос уже отправлен администратору. Ожидай одобрения!"
        )
    else:
        ...
        await update.message.reply_text(
            "Запрос на доступ отправлен администратору. Ожидай одобрения!"
        )
